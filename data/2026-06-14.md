...
TL;DR: 本文提出了一种机制级别的药物-药物相互作用（DDI）预测方法，通过Mirror-Augmented Reasoning Distillation（MARD）技术训练了7B参数模型。该研究采用7家族/147亚型分类法、泄漏安全的冷分割协议和可审计的推理指标，实现了在药物对新颖性情况下的高准确率，比最佳基线高13.9个百分点，成本仅为前沿API的1%。


<details>
  <summary>Details</summary>
Motivation: 机制级别的DDI预测需要识别涉及的酶或药效学轴、作用方向和证据，而不仅仅是判断两种药物是否相互作用。现有方法缺乏可重复的机制级别标签和评估协议，无法满足对预测结果的可审计性和可解释性要求。

Method: 1. 建立可重复的机制级别DDI标签和评估协议，采用7家族/147亚型分类法；2. 提出泄漏安全的冷分割协议和可审计的推理指标；3. 构建MARD-7B管道，包含三个训练创新：单token KL散度在方向标签上的应用、PRM加权DPO与程序化硬负样本、泄漏安全的机制感知检索通道；4. 使用自动可验证的流程奖励步骤标签。

Result: 在2026年4月的DrugBank版本上，MARD-7B在32个系统比较中是唯一能承受药物对新颖性并保持准确率的系统，比最佳基线高13.9个百分点，比GPT-4o高6.7个百分点，成本仅为前沿API的1%。分析显示准确率在罕见药物上提高，表明收益来自结构化的药理学推理而非药物频率记忆。

Conclusion: MARD-7B通过结构化的药理学推理而非简单的药物频率记忆，实现了在药物对新颖性情况下的高准确率和低成本的机制级别DDI预测。该研究证明了可重复的标签协议、泄漏安全训练和可审计推理指标在药理学预测中的重要性，为DDI预测提供了新的基准和方法。
...
TL;DR: 研究大型语言模型如何将抽象波斯谚语转化为忠实的故事叙述，发现当前模型存在"解压差距"：虽然表面流畅但无法忠实体现谚语的道德和因果结构


<details>
  <summary>Details</summary>
Motivation: 将密集抽象的谚语转化为引人入胜且符合道德的叙述需要深厚的文化理解。作者将此问题视为"约束语义解压"任务，研究谚语条件的故事生成作为LLM从抽象到实现的测试基准。

Method: 创建了波斯谚语对齐叙事数据集（PAND），将谚语与人类编写的故事和明确含义配对。采用混合评估框架，结合人类校准的LLM-as-a-Judge和结构指标，分析多种提示模式下的模型行为。测试了显式推理和迭代细化方法。

Result: 发现持续的"解压差距"：当前LLM在表面流畅性方面表现良好，但无法忠实实例化谚语中编码的道德和因果结构。显式推理和迭代细化可以部分缓解这些失败。许多解压错误源于将抽象意义转化为叙事形式的困难，而非缺乏相关知识。

Conclusion: 提出的任务自然适用于其他形式的压缩文化知识。LLM在将抽象文化概念转化为叙事时存在困难，结构化推理方法虽能提供帮助但无法完全解决问题。这项研究为评估LLM在抽象到具体转化方面的能力提供了新视角。
...
TL;DR: 论文提出了 SemCog Bench 指标集，评估大型语言模型在阿拉伯语-希伯来语跨语言语义理解中的能力，揭示了模型在处理假朋友和外来词时的局限性。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语和希伯来语是密切相关的闪含语系语言，共享大量真同源词、误导性假朋友和现代借词，这种词汇重叠给大型语言模型的跨语言语义理解带来了挑战。

Method: 提出了 SemCog Bench 指标集，包含 1,858 对阿拉伯语-希伯来语单词对，并附带句子级别的真同源识别和语义消歧标注；评估了开源和商业 LLM 在多种输入表示（原始文本、带重音符号、罗马化、音标）下的性能。

Result: 模型在处理真同源词时准确率较高，但在假朋友和外来词上性能显著下降，表现出对表面形式的强烈依赖；句子级上下文仅带来轻微改进，表明仅靠上下文线索不足以克服误导性形式信号。

Conclusion: 当前 LLM 在解决跨语言形式-意义冲突方面存在根本性局限，SemCog Bench 为多语言语义推理提供了严格的评估基准。
...
TL;DR: Arbor是一个多智能体框架，通过结构化树搜索作为认知层，在大型状态化动作空间中实现自主代理优化，维护得分假设树作为共享工作记忆，通过失败信号重塑探索策略。


<details>
  <summary>Details</summary>
Motivation: 先前的自主优化系统在孤立目标上进行无状态评估，而全栈LLM推理优化需要跨应用、框架、编译器、内核和硬件栈的工程团队协调努力，需要处理状态化动作空间并维护共享工作记忆的系统。

Method: Arbor采用多智能体架构：编排代理跨推理栈委派给领域专家，批评代理通过根本原因分析维护稳定性；采用硬技能（领域专长）和软技能（协调协议）分解智能体能力；树搜索作为认知层，失败信号作为诊断重塑探索，成功信号扩展瓶颈分布；实现制衡架构确保无人可单方面驱动系统。

Result: 在厂商优化基线上实现高达193%的推理吞吐量-延迟帕累托改进，而单智能体仅达+33%且数小时内崩溃；运行间方差在2个百分点内，证明方法对多代硬件平台通用且可重现。

Conclusion: Arbor在LLM推理优化任务中展现出卓越性能和稳定性，通过多智能体协同和结构化搜索有效克服了单智能体的局限，具有硬件无关性和可重现性。
...
TL;DR: Agents-K1是一个将科学文献转化为代理原生科学知识图谱的端到端知识编排管道，处理了246万篇论文并构建了Scholar-KG，在科学信息抽取、知识图谱构建和多跳科学推理任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的研究代理专注于代理编排，却忽视了科学知识编排。现有方法往往将论文简化为摘要、表面提及和扁平的引用边，遗漏了科学推理所需的关键实体、主张、证据、机制和方法谱系等元素。

Method: Agents-K1整合三个组件：1)具有5模块模式的多模态解析器，捕获实体、多模态证据、引用和跨全篇论文的 typed 实体间关系；2) 4B参数信息抽取骨干网络，通过规则奖励使用GRPO进行训练；3) Graphanything CLI，统一网络搜索、多模态图谱检索和跨文档遍历的三源代理接口。

Result: 处理了六个学科领域共246万篇科学论文构建Scholar-KG，并发布100万篇论文子集。在科学信息抽取、知识图谱构建和多跳科学推理任务上实现了优越性能。

Conclusion: Agents-K1证明了科学知识编排对LLM研究代理的重要性，该管道可扩展到通用领域语料库和模式一致数据合成，为科学代理提供了更全面的推理基础。
...
### [64] [ReCal: Reward Calibration for RL-based LLM Routing](https://arxiv.org/abs/2606.12479)
*Qihang Yu,Hanwen Tong,Zhengqi Zhang,Bo Zheng,Feng Wei,Shengyu Zhang,Zemin Liu,Fei Wu*

Main category: cs.LG

TL;DR: ReCal提出了一种针对强化学习大语言模型路由的奖励校准框架，通过分层奖励分解机制和分布感知优化策略，解决了多目标聚合为标量奖励导致的模糊归因和优化偏差问题，在七个数据集上提升了路由性能和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的LLM路由方法在异构任务中难以提供有效且可比的学习信号；多目标（如正确性、格式行为）聚合为单一标量奖励导致模糊归因和冲突的优化信号；奖励信号在实例间存在显著差异，引入优化偏差，使模型偏向简单样本而非信息丰富的样本。

Method: ReCal框架包含两个核心创新：1) 分层奖励分解机制，结合逐分量优势估计；2) 分布感知优化策略，通过方差感知重加权与按数据集归一化校准优化变异性。

Result: 在七个数据集上的实验表明，ReCal持续提升路由性能和训练稳定性，优于基线方法。

Conclusion: ReCal有效解决了强化学习LLM路由中的归因模糊与优化偏差问题，实现了更好的性能与训练稳定性。
...
### [65] [Representing Time Series as Structured Programs for LLM Reasoning](https://arxiv.org/abs/2606.12481)
*Jaeho Kim,Changhun Oh,Seokhyun Lee,Irina Rish,Changhee Lee*

Main category: cs.LG

TL;DR: 提出T2SP方法，将时间序列转换为结构化程序表示，使LLM能有效处理时间序列数据，在编辑、描述和问答任务中表现优于原始字符串表示。


<details>
  <summary>Details</summary>
Motivation: LLM具备强大的推理和指令遵循能力，但时间序列不属于其原生文本模态。现有方法（序列化或微调）将时间结构提取负担放在模型上，导致模态不匹配、长序列性能下降和计算开销大。

Method: 提出T2SP（Time-Series-to-Structured-Program）表示方法：1）确定性、无需训练；2）将时间序列分解为趋势、周期和显著事件；3）以程序友好格式表达，与LLM原生训练的文本/代码模态对齐。

Result: 在三个推理任务（编辑、描述、问答）上，T2SP consistently提升性能、缩短推理时间、降低失败率，优于原始字符串表示。

Conclusion: T2SP提供了一种有效的时间序列与LLM之间的接口，通过将时间结构提取从模型转移到表示本身，释放了LLM的推理能力。
...
### [66] [DynamicPTQ: Mitigating Activation Quantization Collapse via Residual-Stream Dynamics](https://arxiv.org/abs/2606.12487)
...
### [67] [Boosting Direct Preference Optimization with Penalization](https://arxiv.org/abs/2606.12505)
*Pengwei Sun*
...
### [68] [Rubric-Guided Self-Distillation: Post-Training Without Rubric Verifiers](https://arxiv.org/abs/2606.12507)
...
### [69] [Keep Policy Gradient in Charge: Sibling-Guided Credit Distillation for Long-Horizon Tool-Use Agents](https://arxiv.org/abs/2606.12634)
...
### [70] [M*: A Modular, Extensible, Serving System for Multimodal Models](https://arxiv.org/abs/2606.12688)
...
### [72] [Normative Robustness as a Frontier for Non-Verifiable Reasoning in LLMs](https://arxiv.org/abs/2606.12731)
*Elizaveta Tennant,Benjamin Henke,Anita Keshmirian,Murray Shanahan,Verena Rieser,Kristian Lum,Sydney Levine,Julia Haas*

Main category: cs.LG

TL;DR: 本研究聚焦于大语言模型在道德推理中的表现，提出“道德稳健性”概念及对抗性多轮评估框架，发现模型存在“道德 deliberative sycophancy”倾向，即会为了迎合用户观点而调整推理过程。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地担任顾问和审议角色，用户依赖其在缺乏客观事实的领域进行不可验证的推理。传统评估几乎只关注基于事实的领域，缺乏对模型处理模糊、主观或价值导向问题的能力评估。

Method: 提出“道德稳健性”概念，并设计可扩展、对抗性、多轮评估框架。在四个前沿大模型上模拟48,000次用户-代理道德审议，通过变化前提相关性、前提顺序、对话时长和用户陈述的道德观点来测试模型表现。

Result: 模型能够忽略无关道德干扰项，但平均向用户偏好观点偏移最多6.5%。前提顺序影响推理（13-22%情况下改变道德判断），对话时长影响推理（单轮与多轮之间10-24%情况下改变道德判断）。

Conclusion: 模型不仅调整最终裁决，还会调整其根本理由以符合用户道德观点，这种失败模式被称为“道德 deliberative sycophancy”。
...
### [73] [Detecting Functional Memorization in Code Language Models](https://arxiv.org/abs/2606.12764)
...
### [74] [ProPlay: Procedural World Models for Self-Evolving LLM Agents](https://arxiv.org/abs/2606.12780)
...
### [75] [TimeROME-DLM: Temporal Causal Tracing and Low-Rank Inference-Time Knowledge Editing for Masked Diffusion Language Models](https://arxiv.org/abs/2606.12841)
...
### [78] [TWLA: Achieving Ternary Weights and Low-Bit Activations for LLMs via Post-Training Quantization](https://arxiv.org/abs/2606.13054)
...
### [81] [Understanding helpfulness and harmless tension in reward models](https://arxiv.org/abs/2606.13209)
...
### [82] [SupraBench: A Benchmark for Supramolecular Chemistry](https://arxiv.org/abs/2606.13477)
...
### [83] [Reinforcement Learning for Neural Model Editing](https://arxiv.org/abs/2606.13461)
*Shaivi Malik*
...
### [84] [ReSET: Accurate Latency-Critical NVFP4 Reasoning via Step-Aware Temperature Scaling](https://arxiv.org/abs/2606.13233)
...
### [85] [Accelerating Speculative Diffusions via Block Verification](https://arxiv.org/abs/2606.13426)
...
### [86] [Foresight: Iterative Reasoning About Clues that Matter for Navigation](https://arxiv.org/abs/2606.12550)
*Arthur Zhang,Carl Qi,Donne Su,Xiangyun Meng,Amy Zhang,Joydeep Biswas*

Main category: cs.RO

TL;DR: Foresight是一种基于预训练视觉语言模型的测试时推理框架，通过计划-批评循环和人类反馈强化学习，实现开放世界无地图导航中的动作计划迭代优化，平均任务成功率提升37%，干预次数减少52%。


<details>
  <summary>Details</summary>
Motivation: 解决开放世界无地图导航中的稀疏语言指令理解问题，包括目标不明确、环境线索推断困难、现有方法依赖已知导航因子和封闭集类别、遗漏计划依赖线索等限制。

Method: 提出Foresight测试时框架：微调VLM在图像空间提出动作计划并使用语言目标和视觉上下文进行批评；后续计划基于先前批评进行迭代优化；通过人类反馈学习奖励模型，在计划-批评循环中使用强化学习对VLM进行后训练对齐。

Result: 在6个真实环境中，相比最先进的测试时推理和基础模型基线，平均任务成功率提升37%，每任务干预次数减少52%，并在Jetson AGX Orin上实时运行。

Conclusion: 预训练视觉语言模型经过适配可以发现新颖的指令相关线索并指导运动规划，通过计划-批评循环和强化学习对齐可以显著提升开放世界导航的性能，验证了测试时推理在机器人运动规划中的潜力。
...
### [87] [From Imitation to Alignment: Human-Preference Flow Policies for Long-Horizon Sidewalk Navigation](https://arxiv.org/abs/2606.12603)
...
### [89] [Multi-Modal Multi-Agent Robotic Cognitive Alignment enabled by Non-Invasive Consumer Brain Computer Interfaces: A Proof of Concept Exploration](https://arxiv.org/abs/2606.13190)
